...
def treeplot(tree, node_num):
    ...
    for i in ts.keys():
        if isinstance(tree[first_label][i], dict):
            ...
            root = str(int(root) + 1)
            dot.node(root, str(tree[first_label][i]))
            dot.edge(node_num, root, str(i))
...
def plot_view(myTree):
    ...
    treeplot(myTree, "0")
    # dot.render('task2/result/mytree.gv', view=True)
    dot.render('task2/result/test.gv', view=True)
    # dot.render('task2/result/test_8.gv', view=True)
